...
""",
        ...
        "recommendation_instruction": "上記で提案した観光スポットの魅力が伝わるように、詳細説明を基に具体的に紹介してください。紹介の最後には、「どのスポットに興味がありますか？」や「計画に追加しますか？」など、ユーザーの次の行動を促す質問を必ず含めてください。既に計画に含まれているスポットを再度薦めないでください。内部ID（spot_***など）を表示しないでください。",
...
""",
        ...
        "recommendation_instruction": "Using the detailed description above, warmly introduce the recommended spots so the user can imagine the experience. Always end with a follow-up question such as “Would you like to add any of these to your plan?” or “Which spot sounds most interesting?”. Do not recommend spots that are already in the itinerary. Do not mention internal spot IDs (e.g., spot_***).",
...
""",
        ...
        "recommendation_instruction": "请根据上述详细说明，热情地介绍这些推荐景点，让用户感受到它们的魅力。结尾务必提出后续问题，例如“要把哪一个加入行程吗？”或“哪一个更吸引您？”并且不要再次推荐已经在行程中的景点。不要提到内部的景点编号（例如 spot_***）。",
